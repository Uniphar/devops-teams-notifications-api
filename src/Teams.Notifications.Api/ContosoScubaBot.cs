...
namespace ContosoScuba.Bot
{
    public class ContosoScubaBot : IBot
    {
        ...
        public async Task OnTurnAsync(ITurnContext context, CancellationToken cancellationToken = new CancellationToken())
        {
            if (context.Activity.Type == ActivityTypes.Message)
            {
                ...
                if (!await ChatProxied(context))
                {
                    ...
                    await context.SendActivityAsync(nextMessage, cancellationToken);
                }
            }
        ...
        {
            if (context.Activity.ChannelData is JObject channelData)
            {
                //messages with a "chatwithuserid" in channel data are from a Contoso Scuba instructor, chatting a person who has made a new reservation
                JToken userIdToken = null;
                if (channelData.TryGetValue("chatwithuserid", out userIdToken))
                {
                    ...
                    await ReservationSubscriptionService.ForwardToReservationUser(userId, message, context.Adapter, conversationRef);
            ...
            if (ReservationSubscriptionService.UserIsMessagingSubscriber(context.Activity.From.Id))
            {
                ...
                return await ReservationSubscriptionService.ForwardedToSubscriber(context.Activity.From.Id, message, context.Adapter, CancellationToken.None);
            }
            return false;
        }
        ...
        {
            var resultInfo = new ScubaCardResult();
