...
internal ref struct ValueStringBuilder : IDisposable
{
	...
	private void ExpandBuffer(int newMinimumCapacity)
	{
		...
		var newChars = chars = newBuffer.AsSpan<char>();
		
		chars.Slice(0, length).CopyTo(newChars);
	...
	public void Append([InterpolatedStringHandlerArgument("")] scoped ref InterpolatedStringHandler interpolatedStringHandler)
	{
	}
	...
	{
		ArgumentOutOfRangeException.ThrowIfNegative(count);
		length = int.Clamp(length - count, 0, int.MaxValue);
	}
	...
	public string ToString(StringCache cache) => cache.GetString(chars);
